...
from tests.test_console_api import _Console, _environment  # noqa: F401 -- fixture
...
@requires_exports_schema
def test_an_export_made_from_the_console_is_returned_listed_and_downloadable(client):
    """The founder exporting while acting in a tenant (D-111). The export is
    generated by a platform-admin user who is outside the tenant, so tenant
    RLS can't see their users row; the export must still come back -- it
    500'd on the founder's first test-batch export -- and show as DocFlow
    support in the tenant's own history."""
    with _Console() as console, _ReviewTenant("Acme Test Distributor -- console export") as tenant:
        try:
            document = _approved_document(tenant)
            act = f"/admin/tenants/{tenant.tenant_id}/act/review"
            response = client.post(
                f"{act}/documents/{document}/exports", headers=console.headers(), json={"format": "csv"}
            )
            assert response.status_code == 202, response.text
            export = response.json()["export"]
            assert export["by_docflow_support"] is True and export["generated_by"] is None

            run_export(tenant.tenant_id, UUID(export["id"]))
            status = client.get(f"{act}/exports/{export['id']}", headers=console.headers()).json()
            assert status["export"]["status"] == "ready"
            assert _download(client, status).status_code == 200

            history = client.get(
                f"/review/documents/{document}/exports", headers=tenant.headers()
            ).json()["exports"]
            assert [(row["id"], row["by_docflow_support"]) for row in history] == [(export["id"], True)]
        finally:
            with platform_session() as session:
                session.execute(
                    text("DELETE FROM admin_actions WHERE target_tenant_id = :t"),
