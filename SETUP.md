...
1. In the Supabase dashboard, go to **SQL Editor** → **New query**, paste in the entire contents of `supabase/migrations/0001_foundations.sql`, and run it. This creates the Phase 0 tables.
   ...
   - **Phase 2 matching-foundations update:** repeat this step again with `supabase/migrations/0004_matching_foundations.sql` (adds `buyers`, `buyer_merge_candidates`, `items`, `learned_rules`, `extraction_runs`, the duplicate/change-order columns on `documents`, the buyer link and per-field provenance on `document_headers`/`document_lines`, and `deleted_at` on Phase 1's tables). Same manual-application reason as above. Tests needing these (see `apps/api/tests/conftest.py:requires_matching_schema`) skip cleanly until it's applied -- see DECISIONS.md D-062 for exactly what stays unverified until then.
   - **Phase 2 SKU-matching update:** repeat this step again with `supabase/migrations/0005_sku_matching.sql` (adds the match-result columns to `document_lines`: `matched_item_id`, `match_method`, `match_score`, `matched_uom`, `uom_mismatch`, `match_candidates`, `matched_at`, plus the coherence CHECK and one index). No new tables and no new RLS policies -- `document_lines` already has both from `0002`. Same manual-application reason as above. Tests needing these (see `apps/api/tests/conftest.py:requires_sku_matching_schema`) skip cleanly until it's applied -- see DECISIONS.md D-071 for exactly what stays unverified until then, and for what was deliberately structured so it could be verified beforehand.
   - **Phase 2 validation-and-duplicates update:** repeat this step again with `supabase/migrations/0006_validation_and_duplicates.sql` (adds the `document_warnings` table with its own RLS policies, `documents.change_order_of_document_id` plus the two self-reference CHECK constraints, and the expression index on the normalized PO number). Same manual-application reason as above. Tests needing these (see `apps/api/tests/conftest.py:requires_validation_schema`) skip cleanly until it's applied -- 33 of them, across `apps/api/tests/test_validation.py` and `apps/api/tests/test_duplicates.py`. See DECISIONS.md D-079 for exactly what stays unverified until then. Note that this migration adds no column anywhere that could hold a corrected value, deliberately -- CLAUDE.md Section 10 forbids altering a model-extracted value to make validation pass.
   - **Phase 3 review-and-approval update:** repeat this step again with `supabase/migrations/0007_review_and_approval.sql` (adds the `review_actions` and `document_snapshots` tables with their own RLS policies, the approval columns on `documents` -- `approved_at`, `approved_by`, `approved_snapshot_hash`, `review_started_at` -- the `documents_approved_is_attributable` CHECK, and the `acknowledged_review_action_id` link that `0006` deferred). Same manual-application reason as above. Tests needing these (see `apps/api/tests/conftest.py:requires_review_schema`) skip cleanly until it's applied -- 19 of them, in `apps/api/tests/test_review.py`. See DECISIONS.md D-083 for exactly what stays unverified until then. Note that applying `0006` surfaced two real defects that its skip markers had been hiding (D-080), so treat this slice as written-but-unproven until the migration is in.
   - **Phase 3 audit-trail-ordering fix:** repeat this step again with `supabase/migrations/0008_review_action_sequence.sql` (adds `review_actions.sequence`, a `bigserial`, plus its unique index). Small but not optional: `now()` is transaction start time, so an edit to an approved document and the reopen it causes share a `created_at`, and the old `ORDER BY created_at, id` could render the reopen *before* the edit that caused it. See DECISIONS.md D-084. `requires_review_schema` checks for this column too, so the review tests skip until both `0007` and `0008` are applied.
   - **Phase 3 document previews:** repeat this step again with `supabase/migrations/0009_document_previews.sql` (adds `documents.preview_storage_path`, `preview_media_type` and `preview_kind`). Buyers send Word, Excel, email and TIFF, and no browser renders any of them, so the worker stores a viewable rendering and the API serves it without ever decoding a document itself (Section 7.11 keeps parsing out of the web process). `requires_review_schema` checks for these columns, so the review tests skip until it is applied. See DECISIONS.md D-092.
